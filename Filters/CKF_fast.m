function [Xhist, Phist] = CKF_fast(tdata, Ydata, dxhat0, Phat0, Xref0, mu, J2, Xs, R)
    ...
    for icurr = obs_ind
        if mod(icurr, 500) == 0
            fprintf("step %d\n", icurr)
            err = sum((Xhist(1:3,1:iprev)-Xtrue(1:3,1:iprev)).^2);
            [maxerr, imax] = max(err);
            fprintf("Max position error = %.3f km at t = %d s\n", maxerr, tdata(imax))
        end
        ...
        dy = Y- [rho; rhod];
        Htilde = Htilde_sc_rho_rhod(Xref_aug(1:3,icurr), Xref_aug(4:6,icurr), ...
